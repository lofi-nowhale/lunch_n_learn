...
RSpec.describe LearningResourceSerializer do
  ...
  describe "sad path" do
    it "returns an empty hash for video and an empty array for image if the country resources do not exist" do
      VCR.use_cassette("learning_resource_serializer_sad") do
        resource = LearningResourceFacade.new.learning_resource("asdfaasdf")
        ...
        expect(serialized_learning_resource[:data]).to have_key(:attributes)
        expect(serialized_learning_resource[:data][:attributes]).to have_key(:country)
        expect(serialized_learning_resource[:data][:attributes][:country]).to eq("asdfaasdf")
        
        expect(serialized_learning_resource[:data][:attributes]).to have_key(:video)
        expect(serialized_learning_resource[:data][:attributes][:video]).to eq({})
        
        expect(serialized_learning_resource[:data][:attributes]).to have_key(:images)
        expect(serialized_learning_resource[:data][:attributes][:images]).to be_an Array
        expect(serialized_learning_resource[:data][:attributes][:images].count).to eq(0)
      end
    end
  end
end